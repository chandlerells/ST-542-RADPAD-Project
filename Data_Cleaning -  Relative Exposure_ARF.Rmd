...
Initial look at the distribution of each Personnel Type 
```{r}
hist_0 <- ggplot(tib_)+ 
...
```
Look at the Personnel Type after Box Cox transformations 
```{r}
...
#fit zero-inflated gamma model
zig.fit5 <- glmmTMB(Relative_Dose ~ RADPAD*Procedure_Type + Weight + Time,
...
plot(DHARMa:: simulateResiduals(fittedModel = zig.fit5))
```
